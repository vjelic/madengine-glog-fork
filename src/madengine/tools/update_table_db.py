#!/usr/bin/env python
"""Module to update tables in the database.

This module provides the functions to update tables in the database.
...
class UpdateTable:
    ...
    def run(self, table_name: str = "dlm_table") -> None:
        ...
        if "localhost" in self.ssh_hostname or "127.0.0.1" in self.ssh_hostname:
            try:
                ...
                LOGGER.error(f"Error updating table in the local database: {error}")
                return self.status
        else:
            try:
                self.remote_db()
    ...
    def local_db(self) -> None:
        ...
        Raises:
            Exception: An error occurred updating the table.
        """
        print("Updating table in the local database")
    ...
    def remote_db(self) -> None:
        ...
        """
        print("Updating table in the remote database")
        ...
        csv_file_path_remote = os.path.basename(self.args.csv_file_path)
        model_json_path_remote = os.path.basename(self.args.model_json_path)
        ...
        if not os.path.exists(self.args.csv_file_path):
            print(f"File {self.args.csv_file_path} does not exist")
            return
        sftp_client.put(self.args.csv_file_path, csv_file_path_remote)
        # check if the file exists
        if os.path.exists(self.args.model_json_path):
            sftp_client.put(self.args.model_json_path, model_json_path_remote)
