...
class TestValidateAdditionalContext:
    ...
    def test_validate_additional_context_invalid_gpu_vendor(self):
        ...
        with patch('madengine.mad_cli.console') as mock_console:
            with pytest.raises(typer.Exit) as exc_info:
                validate_additional_context('{"gpu_vendor": "INVALID", "guest_os": "UBUNTU"}')
    ...
    def test_validate_additional_context_invalid_guest_os(self):
        ...
        with patch('madengine.mad_cli.console') as mock_console:
            with pytest.raises(typer.Exit) as exc_info:
                validate_additional_context('{"gpu_vendor": "AMD", "guest_os": "INVALID"}')
...
class TestBuildCommand:
    ...
    def test_build_command_invalid_context(self):
        ...
        result = self.runner.invoke(app, [
            "build",
            "--tags", "dummy",
            "--additional-context", "invalid json"
        ])
    ...
    @patch('madengine.mad_cli.DistributedOrchestrator')
    @patch('madengine.mad_cli.validate_additional_context')
    def test_build_command_exception_handling(self, mock_validate, mock_orchestrator_class):
...
class TestRunCommand:
    ...
    @patch('madengine.mad_cli.DistributedOrchestrator')
    @patch('madengine.mad_cli.validate_additional_context')
    def test_run_command_full_workflow(self, mock_validate, mock_orchestrator_class, mock_exists):
    ...
    @patch('madengine.mad_cli.DistributedOrchestrator')
    @patch('madengine.mad_cli.validate_additional_context')
    def test_run_command_build_failure(self, mock_validate, mock_orchestrator_class, mock_exists):
...
class TestGenerateAnsibleCommand:
    ...
    def test_generate_ansible_manifest_not_found(self, mock_exists):
        ...
        result = self.runner.invoke(app, [
            "generate", "ansible",
            "--manifest-file", "missing_manifest.json"
        ])
...
class TestEdgeCases:
    ...
    def test_build_empty_tags(self):
        ...
        assert result.exit_code in [ExitCode.SUCCESS, ExitCode.BUILD_FAILURE, ExitCode.INVALID_ARGS]
    ...
    def test_run_zero_timeout(self):
        ...
        assert result.exit_code != ExitCode.INVALID_ARGS or "Timeout" not in result.stdout
    ...
    def test_context_file_and_string_both_provided(self, mock_validate):
        ...
        try:
            result = self.runner.invoke(app, [
                "build",
                "--additional-context", context_json,
                "--additional-context-file", temp_file
            ])
